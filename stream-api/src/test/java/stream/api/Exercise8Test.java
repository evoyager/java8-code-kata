...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        List<Item> itemsCustomerWantToBuyList = customerStream.flatMap(c -> c.getWantToBuy().stream()).collect(Collectors.toList());
        List<String> itemOnSaleNameList = shopStream.flatMap(s -> s.getItemList().stream()).map(Item::getName).collect(Collectors.toList());

        List<String> itemCustomerWantToBuyAndOnSaleNameList = itemsCustomerWantToBuyList.stream()
                .filter(itemCustomerWantToBuy -> itemOnSaleNameList.contains(itemCustomerWantToBuy.getName()))
                .map(Item::getName).collect(Collectors.toList());

        Set<String> itemSetNotOnSale = itemsCustomerWantToBuyList.stream().filter(itemCustomerWantToBuy ->
                        !itemCustomerWantToBuyAndOnSaleNameList.contains(itemCustomerWantToBuy.getName()))
                .map(Item::getName)
    ...
    public void havingEnoughMoney() {
        ...
        List<String> onSaleItemNames = onSale.stream().map(Item::getName).collect(Collectors.toList());
        Predicate<Customer> havingEnoughMoney = customer -> customer.getBudget() >=
                customer.getWantToBuy().stream()
                        .mapToInt(item -> {
                            if (onSaleItemNames.contains(item.getName())) {
                                return onSale.stream().filter(onSaleItem -> onSaleItem.getName().equals(item.getName()))
